...
for method in main_methods:
    ...
    ax.text(x, y, method, ha='center', va='center', fontsize=20,
            bbox=dict(boxstyle='round,pad=0.2', facecolor='white', edgecolor='#2E86AB', linewidth=1.5),
            zorder=4)
...
# Add DFT label
ax.text(
    positions['DFT_box'][0] + 0.3, max(dft_y_coords) + 0.45,
    'Density Functional \nTheory (DFT)',
    ha='center', va='bottom', fontweight='bold',
    color='#A23B72', zorder=4, fontsize=BOX_LABEL_FONT_SIZE,
)
...
for method in dft_methods:
    ...
    ax.text(x + 0.09, y + 0.05, method, ha='right', va='bottom',
            color='#A23B72', zorder=4, fontsize=BOX_CONTENT_FONT_SIZE)
...
# Add ab initio label
ax.text(
    positions['ab_initio_box'][0] + 0.05, max(ab_y_coords) + 0.45,
        'Ab Initio \n(Wavefunction)',
        ha='center', va='bottom', fontweight='bold',
        color='#C73E1D', zorder=4, fontsize=BOX_LABEL_FONT_SIZE)
...
for method in ab_initio_methods:
    ...
    ax.text(x + 0.09, y + 0.05, method, ha='right', va='bottom',
            color='#C73E1D', zorder=4, fontsize=BOX_CONTENT_FONT_SIZE)
